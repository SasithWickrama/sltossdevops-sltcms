...
function search() {
	...
	self.location='slt_ret.php?rt=' + val ;
}
</script>   
</head>
...
$so_list = retso_slt($temp2[0],$temp2[1],$temp2[2],$temp2[3]);;
...
echo"	<div class=\"row\">
                ...
                <div class=\"form-group\">
                 <a href=\"dashreports/{$temp2[0]}_Return.csv\" download><img src=\"dist/img/Excel.ico\" width=\"30\" height=\"30\"/>
                  <label >EXCEL</label></a>
     ...
     $HEADER = "RTOM,LEA,SOD NO,Circuit,Service Type,Order Type,Task Name,Contractor,Received Date,Return Date,Reason  \n";    
    while($row=oci_fetch_array($so_list))
    {		
            $HEADER = $HEADER . "{$row[0]},{$row[1]},{$row[2]},{$row[3]},{$row[4]},{$row[5]},{$row[6]},{$row[7]},{$row[10]},{$row[8]},{$row[9]}\n";	
    ...
    $File = "dashreports/{$temp2[0]}_Return.csv";
