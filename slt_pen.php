...
function search() {
	...
	self.location='slt_pen.php?rt=' + val ;
}
</script>   
</head>
...
$temp2 = explode('_',$rt);
$so_list = penso_slt($temp2[0],$temp2[1],$temp2[2],$temp2[3]);
...
echo"	<div class=\"row\">
                ...
                <div class=\"form-group\">
                 <a href=\"dashreports/{$temp2[0]}_pendingso.csv\" download><img src=\"dist/img/Excel.ico\" width=\"30\" height=\"30\"/>
                  <label >EXCEL</label></a>
     ...
     $HEADER = "RTOM,LEA,SOD NO,Circuit,Service Type,Order Type,Task Name,Package,Contractor,Received Date,Status  \n";    
    while($row=oci_fetch_array($so_list))
    {		
            $HEADER = $HEADER . "{$row[0]},{$row[1]},{$row[2]},{$row[3]},{$row[4]},{$row[5]},{$row[6]},{$row[7]},{$row[8]},{$row[9]},{$row[10]}\n";	
    ...
    $File = "dashreports/{$temp2[0]}_pendingso.csv";
