...
<script type="text/javascript">
	  ...
	  if(status == '3'){
		...
		var r = confirm("Are you sure you want to cancel sod!");
		  ...
		  if (r == true) {
			...
			$("#modal_body").html('<div class="form-group  row">'+
								'<label class="col-sm-3 col-form-label" style="font-size:14px;">Drop Wire*</label>'+
								'</div>'+
								'<div class="form-group  row">'+
								'<label class="col-sm-3 col-form-label" style="font-size:14px;">Unit Designator</label>'+
								'<div class="col-sm-3">'+
								'<input type="text" id="so_no" value="'+so_no+'" style="display:none">'+
								'<input type="text" id="voice_no" value="'+voice_No+'" style="display:none">'+
								'<input type="text" id="group" value="'+group+'" style="display:none">'+
								...
								'<input type="text" id="taskName" value="'+taskName+'" style="display:none">'+
								'<select name="unit_desig" id="unit_desig" class="form-control">'+
								'<option value="PSTN-DW">PSTN-DW</option>'+
								'</select>'+
								'</div>'+
								'<label class="col-sm-2 col-form-label" style="font-size:14px;">Length*</label>'+
								'<div class="col-sm-3">'+
								'<input type="number" id="dwlength" class="form-control">'+
								'</div>'+
								'</div>'+
								'<div class="form-group  row">'+
								'<label class="col-sm-3 col-form-label" style="font-size:14px;">Other Material</label>'+
								'<div class="col-sm-3">'+
								...
								'</div>'+
								'<div class="col-sm-3">'+
								'<i class="fa fa-plus-circle" onClick="addData()" style="font-size:26px"></i>'+
								'</div>'+
								'</div>'+
								'<div class="form-group  row">'+
								'<label class="col-sm-3 col-form-label" style="font-size:14px;">&nbsp;</label>'+
								'<div class="col-sm-6" style="font-size:13px;">'+
								'<table class="table table-striped table-bordered" id="tblother">'+
								'</table>'+
								'</div>'+
								'</div>'+
								'<div class="form-group  row">'+
	...
	function complete_ABCAB(){
		...
		$.ajax({
         type: "POST",
         data: {voice_no:voice_no,so_no:so_no,rtom:rtom,stype:stype,group:group,taskName:taskName,dwlength:dwlength,unit_desig:unit_desig},
	...
	function ins_ABCAB_omat(){
		
		var tbl = document.getElementById('tblother');
		var voice_no = $('#voice_no').val();
		var so_no = $('#so_no').val();
		var rtom = $('#rtom').val();
		var stype = $('#stype').val();
		...
		for(var i=0; i<tbl.rows.length; i++){
		  ...
		  var other_mat = tbl.rows[i].cells[0].innerHTML;
		  
		  $.ajax({
			 type: "POST",
			 data: {voice_no:voice_no,so_no:so_no,rtom:rtom,stype:stype,group:group,taskName:taskName,other_mat:other_mat},
			 url: "./function.php?r=31",
			 success: function(res){
...
function addData(){
	...
	for(var i=0; i<tbl.rows.length; i++){
	  ...
	  var selectedMat = tbl.rows[i].cells[0].innerHTML;
	    
		if(selectedMat == other_mat){
	...
	if(other_mat == ''){
		...
		return false;
		 ...
		 $('#tblother').append('<tr><td style="width:400px;">'+other_mat+'</td><td><i class="fa fa-times-circle" style="font-size:24px" onClick="removeData(this)"></i></td></tr>');
	}
}
...
function removeData(r){
    ...
    document.getElementById("tblother").deleteRow(i);
