...
{
    $sql = "select distinct * from (SELECT DISTINCT CW.CON_SERO_ID,CW.CON_PSTN_NUMBER,
TO_CHAR(CW.CON_DATE_TO_CONTRACTOR, 'mm/dd/yyyy hh:mi:ss AM'), CW.CON_STATUS,CW.CON_WORO_SERVICE_TYPE,CW.CON_WORO_ORDER_TYPE,CW.CON_WORO_AREA,
...
function validate_user($uname, $con)
{
    ...
    * CON_MGT_USER_PRV_LEVEL,CON_MGT_USER_KEY from CONTRACTOR_MGT_USERS where CON_MGT_USER_NAME = '{$uname}'";
    ...
    $sql ="select CON_MGT_USER_NAME,CON_MGT_CONTRACTOR, CON_MGT_PW_HASH, 
     CON_MGT_USER_PRV_LEVEL,SLT_AREA ,CON_MGT_CON_COODINATOR from CONTRACTOR_MGT_USERS where CON_MGT_CONTRACTOR = '{$con}' AND CON_MGT_USER_NAME = :CON_MGT_USER_NAME";
...
function insertsms($pwd,$uname,$phn)
{
    $sql = "INSERT INTO SLT010563.SMSMSGS1 (
   SEND_TPNO, SEND_MSG, SEND_SUCS, SENT_OWNER, SMS_ID, DATE_INS,SEND_USER)
VALUES ('$phn','your username : $uname and Password : $pwd','0','OSS','$phn',SYSDATE,'SLTCMS')";
...
function nc_meterial2qty($so_id)
{
    $sql ="select count(*) from CONTRACTOR_COPPER_MET where SOID = '{$so_id}'";
    $oraconn = OracleConnection();
    $ncm = oci_parse($oraconn, $sql);
...
function co_inv_listsltchk($a)
{
    $sql = "SELECT * FROM CONTRACTOR_INVOICE_DATA where INVOICE_NO = '$a' ";
...
if(isset($_POST["q"]) && $_POST["q"]== 'deleteinv' ){
$inv=$_POST["inv"];    


  $sql="update CONTRACTOR_INV_PASSCHK set INV_NO=null , INV_FLAG = null where INV_NO = '$inv'";

  $sql1="update CONTRACTOR_INV_PASSED set INV_NO=null , INV_FLAG = null where INV_NO = '$inv'";

  $sql2="delete from CONTRACTOR_INVOICE_NUMWISE where INVNO = '$inv'";

  $sql3="delete from QUALITY_SEVERTY where INV_NO = '$inv'";

  $sql5="delete from CONTRACTOR_INVOICE_DATA where INVOICE_NO = '$inv'";

  $sql4="delete from CONTRACTOR_INVNO_REF where INVNO = '$inv'";
      
  $sql4="delete from CONTRACTOR_INVOICE_DATE where INVNO = '$inv'";  
    ...
    if(oci_execute($con_comp))
    {
		$msg =  'update CONTRACTOR_INV_PASSCHK : '.$inv;
		log_all($user,$inv,$msg);
		$con_comp1= oci_parse($oraconn, $sql1);
		if(oci_execute($con_comp1))
		{
			$msg1 =  'update CONTRACTOR_INV_PASSED : '.$inv;
			log_all($user,$inv,$msg1);
			$con_comp2= oci_parse($oraconn, $sql2);
			if(oci_execute($con_comp2))
			{
				$msg2 =  'delete from CONTRACTOR_INVOICE_NUMWISE : '.$inv;
				log_all($user,$inv,$msg2);
				$con_comp3= oci_parse($oraconn, $sql3);
				if(oci_execute($con_comp3))
				{
					$msg3 =  'delete from QUALITY_SEVERTY : '.$inv;
					log_all($user,$inv,$msg3);
					$con_comp4= oci_parse($oraconn, $sql4);
					if(oci_execute($con_comp4))
					{
						$msg4 =  'delete from CONTRACTOR_INVOICE_DATA : '.$inv;
						log_all($user,$inv,$msg4);
						$con_comp5= oci_parse($oraconn, $sql5);
						if(oci_execute($con_comp5))
						{
                            
                            $msg5 =  'delete from CONTRACTOR_INVNO_REF : '.$inv;
    						log_all($user,$inv,$msg5);
    						$con_comp6= oci_parse($oraconn, $sql6);
    						if(oci_execute($con_comp6))
    						{
    							
    							$msg6 =  'delete from CONTRACTOR_INVOICE_DATE : '.$inv;
    							log_all($user,$inv,$msg6);
							     echo  "success";
    							
    						}else
    						{
    							$err = oci_error($con_comp6);
    							$e =  $err['message'];
    							echo $e;
    						}
...
if(isset($_POST["q"]) && $_POST["q"]== 'deletemetqty' ){
$met=$_POST["met"];    
$ser=$_POST["ser"];
$cir=$_POST["cir"];
$metid=$_POST["metid"];
...
if(isset($_POST["q"]) && $_POST["q"]== 'addmetqty' ){
$met=$_POST["met"];
$sn=$_POST["sn"];
$type=$_POST["type"];
$ser=$_POST["ser"];
...
function setinv($a,$b,$c,$d)
{
    $sql= "select trim(TO_CHAR(max(SEQNO)+1,'000'))
        from CONTRACTOR_INVNO_REF
        where CON_NAME  = '$a' and SER_TYPE = '$b' and AREA= '$c' and INVYEAR = '$d' ";
...
function invdate($a,$b,$c,$d){

    $sql= "INSERT INTO OSS_DEV_01.CONTRACTOR_INVOICE_DATE (INVNO, FROMDATE, TODATE,INDATE, INUSER) VALUES ('$a','$b','$c',sysdate,'$d')";
...
function getftthcabclosesltqty($a)
{
    $sql ="select distinct a.SO_NUM,a.S_TYPE,to_char(b.CON_SO_DATE_RECEIVED, 'mm/dd/yyyy hh:mi:ss AM'),A.VOICENUMBER,B.CON_AREA, C.CON_WORO_ORDER_TYPE,
        C.CON_CUS_NAME,C.CON_TEC_CONTACT, d.CON_ADDE_STREETNUMBER||' '||d.CON_ADDE_STRN_NAMEANDTYPE||' '||d.CON_ADDE_SUBURB||' '||d.CON_ADDE_CITY,
        e.CON_OSP_DP_NAME,e.CON_OSP_DP_LOOP,e.CON_OSP_PHONE_CLASS,e.CON_OSP_PHONE_COLOUR,e.CON_PHN_PURCH,c.CON_WORO_DISCRIPTION,c.CON_WORO_TASK_NAME,
        b.CON_PENALTY,b.CON_DLY_DAYS,to_char(b.CON_SO_COM_DATE, 'mm/dd/yyyy hh:mi:ss AM'),b.CON_APPROVE
        from CON_CLARITY_SOLIST a, CONTRACTOR_INV_PASSCHK b, CONTRACTOR_WORK_ORDERS c,CONTRACTOR_SERVICE_ADDRESS d,CONTRACTOR_OSP_DATA e
        where A.SO_NUM = C.CON_SERO_ID
        and B.CON_SO_ID = C.CON_SERO_ID
        and B.CON_SO_ID = D.CON_ADDE_SERO_ID
        and B.CON_SO_ID = E.CON_OSP_SERO_ID(+)
        ...
        C.CON_CUS_NAME,C.CON_TEC_CONTACT, d.CON_ADDE_STREETNUMBER||' '||d.CON_ADDE_STRN_NAMEANDTYPE||' '||d.CON_ADDE_SUBURB||' '||d.CON_ADDE_CITY,
        e.CON_OSP_DP_NAME,e.CON_OSP_DP_LOOP,e.CON_OSP_PHONE_CLASS,e.CON_OSP_PHONE_COLOUR,e.CON_PHN_PURCH,c.CON_WORO_DISCRIPTION,c.CON_WORO_TASK_NAME ,b.CON_PENALTY,b.CON_DLY_DAYS,to_char(b.CON_SO_COM_DATE, 'mm/dd/yyyy hh:mi:ss AM'),b.CON_APPROVE
        from CONTRACTOR_INV_PASSCHK b, CONTRACTOR_WORK_ORDERS c,CONTRACTOR_SERVICE_ADDRESS d,CONTRACTOR_OSP_DATA e
        where B.CON_SO_ID = C.CON_SERO_ID
        and B.CON_SO_ID = D.CON_ADDE_SERO_ID
        and B.CON_SO_ID = E.CON_OSP_SERO_ID(+)
        and B.CON_SO_ID ='$a'
...
CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ...
   trunc( sysdate-CW.CON_DATE_TO_CONTRACTOR ) , 
       trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24, 24 ) ) , 
       trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24*60, 60 ) ), 
 trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24*60*60, 60 ) )
FROM CONTRACTOR_WORK_ORDERS CW, CONTRACTOR_NEW_CON CO, CON_CLARITY_SOLIST CS
...
CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ...
   (SELECT DISTINCT CON_OSP_PHONE_COLOUR FROM CONTRACTOR_OSP_DATA WHERE CON_OSP_SERO_ID = CON_SERO_ID)
   ELSE ''
   END AS PKG,CW.CON_NAME, TO_CHAR(CW.CON_DATE_TO_CONTRACTOR, 'mm/dd/yyyy hh:mi:ss AM'), CW.CON_STATUS,trunc( sysdate-CW.CON_DATE_TO_CONTRACTOR ), 
       trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24, 24 ) ), 
       trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24*60, 60 ) ) , 
 trunc( mod( (sysdate-CW.CON_DATE_TO_CONTRACTOR)*24*60*60, 60 ) )
FROM CONTRACTOR_WORK_ORDERS CW, CONTRACTOR_NEW_CON CO, CON_CLARITY_SOLIST CS
...
CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ...
   CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ( SELECT DISTINCT CON_FTTH_ONT_SN FROM CONTRACTOR_FTTH_DATA WHERE CON_FTTH_SERO_ID = CON_SERO_ID)
    WHEN CON_WORO_SERVICE_TYPE = 'AB-CAB' THEN 
   (SELECT DISTINCT '' FROM CONTRACTOR_OSP_DATA WHERE CON_OSP_SERO_ID = CON_SERO_ID)
   ELSE ''
   END AS SN
...
union all
select distinct cw.CON_WORO_AREA,op.CON_EX_AREA,cw.CON_SERO_ID,cw.CON_PSTN_NUMBER,cw.CON_WORO_SERVICE_TYPE,
cw.CON_WORO_ORDER_TYPE,cw.CON_WORO_TASK_NAME ,
CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ( SELECT DISTINCT CON_FTTH_PKG FROM CONTRACTOR_FTTH_DATA WHERE CON_FTTH_SERO_ID = CON_SERO_ID)
    WHEN CON_WORO_SERVICE_TYPE = 'AB-CAB' THEN 
   (SELECT DISTINCT CON_OSP_PHONE_COLOUR FROM CONTRACTOR_OSP_DATA WHERE CON_OSP_SERO_ID = CON_SERO_ID)
...
TO_CHAR(CO.CON_SO_DATE_RECEIVED , 'mm/dd/yyyy hh:mi:ss AM'), CON_OSP_DP_NAME||' - '||CON_OSP_DP_LOOP,
CASE WHEN CON_WORO_SERVICE_TYPE = 'AB-FTTH' THEN 
   ( SELECT DISTINCT CON_FTTH_ONT_SN FROM CONTRACTOR_FTTH_DATA WHERE CON_FTTH_SERO_ID = CON_SERO_ID and ROWNUM <= 1)
    WHEN CON_WORO_SERVICE_TYPE = 'AB-CAB' THEN 
   (SELECT DISTINCT '' FROM CONTRACTOR_OSP_DATA WHERE CON_OSP_SERO_ID = CON_SERO_ID)
   ELSE ''
   END AS SN
...
AND TO_DATE('$c 11:59:59 PM','mm,dd,yyyy:hh:mi:ss pm')";*/
$sql ="select * from (
SELECT DISTINCT CW.CON_WORO_AREA,CS.LEA,
   CW.CON_SERO_ID,CW.CON_PSTN_NUMBER,CW.CON_WORO_SERVICE_TYPE,CW.CON_WORO_ORDER_TYPE,CW.CON_WORO_TASK_NAME,
'' TEST1,CW.CON_NAME, TO_CHAR(CO.CON_SO_COM_DATE, 'mm/dd/yyyy hh:mi:ss AM'), CW.CON_STATUS,
   TO_CHAR(CO.CON_SO_DATE_RECEIVED , 'mm/dd/yyyy hh:mi:ss AM'), CON_OSP_DP_NAME||' - '||CON_OSP_DP_LOOP,
   '' TEST2
FROM CONTRACTOR_WORK_ORDERS CW, CONTRACTOR_NEW_CON CO, CON_CLARITY_SOLIST CS,CONTRACTOR_OSP_DATA op
WHERE CO.CON_SO_STATUS = CW.CON_STATUS
...
AND CW.CON_SERO_ID = CO.CON_SO_ID 
and CW.CON_SERO_ID = op.CON_OSP_SERO_ID
and CW.CON_WORO_ID = op.CON_OSP_WORO_ID
 and CO.CON_SO_ID = op.CON_OSP_SERO_ID
AND CW.CON_WORO_TASK_NAME = CO.CON_TASK
 and CO.CON_STATUS = '2'
AND CO.CON_SO_ID = CS.SO_NUM
...
cw.CON_WORO_ORDER_TYPE,cw.CON_WORO_TASK_NAME ,
'' TEST1,cw.CON_NAME ,to_char(CO.CON_SO_COM_DATE, 'mm/dd/yyyy hh:mi:ss AM'), cw.CON_STATUS,
TO_CHAR(CO.CON_SO_DATE_RECEIVED , 'mm/dd/yyyy hh:mi:ss AM'), CON_OSP_DP_NAME||' - '||CON_OSP_DP_LOOP,'' TEST2
from CONTRACTOR_WORK_ORDERS cw, CONTRACTOR_NEW_CON co ,CONTRACTOR_OSP_DATA op
 where co.CON_SO_STATUS = cw.CON_STATUS
and cw.CON_STATUS IN ('COMPLETED')
 and CW.CON_WORO_SERVICE_TYPE = '$d'
...
function retso_slt($a,$b,$c,$d)
{
		...
		TO_CHAR(CW.CON_DATE_TO_CONTRACTOR, 'mm/dd/yyyy hh:mi:ss AM')
...
function getmaxsod($a){
  ...
  $sql= " select distinct CON_SO_ID,CON_CIRCUIT_NO, CON_CONTRACTOR,CON_AREA  from CONTRACTOR_NEW_CON 
...
function getmaxsodchk($a,$b){

        
   $sql= " select distinct CON_SO_ID,CON_CIRCUIT_NO, CON_CONTRACTOR,CON_AREA  from CONTRACTOR_NEW_CON 
        where CON_CIRCUIT_NO = '$a' and CON_CONTRACTOR = '$b' and 
        CON_SO_STATUS IN ('ASSIGNED', 'INPROGRESS', 'COMPLETED', 'RETURNED', 'RE_RETURNED')";     
...
function getmaxsodqty($a){


  $sql= " select CON_SO_ID from CONTRACTOR_INV_PASSCHK 
        where CON_CIRCUIT_NO = '$a' and 
        CON_SO_STATUS  = 'COMPLETED'
		and INV_NO is null
...
function getteamrt($a, $b){

if($a== 'ALL'){
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where CONTRACTOR = '$b' and TEAM_DATE  = TO_DATE(TO_CHAR(sysdate, 'MM/DD/YYYY'), 'MM/DD/YYYY')";
}else{
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where CONTRACTOR = '$b' and area = '$a' and TEAM_DATE = TO_DATE(TO_CHAR(sysdate, 'MM/DD/YYYY'), 'MM/DD/YYYY') ";
}
...
function getteamrtslt($a){

if($a== 'SLTQC'){
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where  TEAM_DATE  = TO_DATE(TO_CHAR(sysdate, 'MM/DD/YYYY'), 'MM/DD/YYYY')";
}else{
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where  area = '$a' and TEAM_DATE = TO_DATE(TO_CHAR(sysdate, 'MM/DD/YYYY'), 'MM/DD/YYYY') ";
}
...
function getteamrtsc($a, $b,$c){

if($a == 'ALL'){
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where CONTRACTOR = '$c'  and TEAM_DATE BETWEEN TO_DATE('$b 12:00:00 AM','mm,dd,yyyy:hh:mi:ss pm') 
AND TO_DATE('$b 11:59:59 PM','mm,dd,yyyy:hh:mi:ss pm') and TEAM_FLAG = '1' ";
}else{
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where CONTRACTOR = '$c' and area = '$a' and TEAM_DATE BETWEEN TO_DATE('$b 12:00:00 AM','mm,dd,yyyy:hh:mi:ss pm') 
AND TO_DATE('$b 11:59:59 PM','mm,dd,yyyy:hh:mi:ss pm') and TEAM_FLAG = '1' ";
...
function getteamrtscslt($a, $b){

if($a == 'SLTQC'){
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where TEAM_DATE BETWEEN TO_DATE('$b 12:00:00 AM','mm,dd,yyyy:hh:mi:ss pm') 
AND TO_DATE('$b 11:59:59 PM','mm,dd,yyyy:hh:mi:ss pm') and TEAM_FLAG = '1' ";
}else{
	$sql=  " select AREA, CONTRACTOR, TO_CHAR(TEAM_DATE, 'MM/DD/YYYY'), S_TYPE, TEAM_COUNT, EN_USER, TEAM_FLAG,TEAM_DATE
	from CONTRACTOR_TEAMS where  area = '$a' and TEAM_DATE BETWEEN TO_DATE('$b 12:00:00 AM','mm,dd,yyyy:hh:mi:ss pm') 
AND TO_DATE('$b 11:59:59 PM','mm,dd,yyyy:hh:mi:ss pm') and TEAM_FLAG = '1' ";
...
$stype=$_POST["stype"];
  ...
  $sql= "DELETE FROM CONTRACTOR_TEAMS WHERE AREA= '$area' and CONTRACTOR= '$con' and TEAM_DATE = '$tdate' and S_TYPE = '$stype'";

    $oraconn = OracleConnection();
    $con_comp= oci_parse($oraconn, $sql);
    if(oci_execute($con_comp))
    {
     log_all($user,'Team Delete '.$con);
    echo  "success";
...
VALUES ('$rt','$con',TO_DATE(TO_CHAR(sysdate, 'MM/DD/YYYY'), 'MM/DD/YYYY'),'$stype','$steam', '$user', '0') ";
...
$stype=$_POST["stype"];
  ...
  $sql= "update  CONTRACTOR_TEAMS set TEAM_FLAG = '1' WHERE AREA= '$area' and CONTRACTOR= '$con' and TEAM_DATE = '$tdate' and S_TYPE = '$stype'";
